...
static int _get_usbfs_fd(struct libusb_device *dev, mode_t mode, int silent)
{
	...
	if (errno == ENOENT) {
		if (!silent) 
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "File doesn't exist, wait %d ms and try again", delay/1000);
	...
	if (!silent) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "libusb couldn't open USB device %s: %s",
			 path, strerror(errno));
		if (errno == EACCES && mode == O_RDWR)
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "libusb requires write access to USB "
				      "device nodes.");
	}
...
static int _is_usbdev_entry(struct dirent *entry, int *bus_p, int *dev_p)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "found: %s", entry->d_name);
	if (bus_p != NULL)
		*bus_p = busnum;
	if (dev_p != NULL)
		*dev_p = devnum;
 ...
 * from reading /dev on newer devices. */
	...
	if (ret != NULL)
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "found usbfs at %s", ret);
...
static clockid_t find_monotonic_clock(void)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "monotonic clock doesn't work, errno %d", errno);
#endif
...
static int op_init(struct libusb_context *ctx)
{
	...
	if (!usbfs_path) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "could not find usbfs");
	...
	if (supports_flag_bulk_continuation == -1) {
		...
		if (supports_flag_bulk_continuation == -1) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "error checking for bulk continuation support");
			return LIBUSB_ERROR_OTHER;
		}
	}
	...
	if (supports_flag_bulk_continuation)
        {
	//	__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "bulk continuation flag supported");
        }
	if (-1 == supports_flag_zero_packet) {
		/* zero length packet URB flag fixed since Linux 2.6.31 */
		supports_flag_zero_packet = kernel_version_ge(2,6,31);
		if (-1 == supports_flag_zero_packet) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "error checking for zero length packet support");
			return LIBUSB_ERROR_OTHER;
		}
	}

	if (supports_flag_zero_packet)
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "zero length packet flag supported");

	if (-1 == sysfs_has_descriptors) {
		/* sysfs descriptors has all descriptors since Linux 2.6.26 */
		sysfs_has_descriptors = kernel_version_ge(2,6,26);
		if (-1 == sysfs_has_descriptors) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "error checking for sysfs descriptors");
			return LIBUSB_ERROR_OTHER;
		}
	}
	...
	if (-1 == sysfs_can_relate_devices) {
		...
		if (-1 == sysfs_can_relate_devices) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "error checking for sysfs busnum");
			return LIBUSB_ERROR_OTHER;
		}
	}
	...
	if (sysfs_can_relate_devices || sysfs_has_descriptors) {
		...
		if (r != 0 || !S_ISDIR(statbuf.st_mode)) {
			__android_log_print(ANDROID_LOG_WARN, "LibUSB", "sysfs not mounted");
	...
	if (sysfs_can_relate_devices)
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "sysfs can relate devices");

	if (sysfs_has_descriptors)
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "sysfs has complete descriptors");
	...
	if (r == LIBUSB_SUCCESS) {
		...
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "error starting hotplug event monitor");
...
static int _open_sysfs_attr(struct libusb_device *dev, const char *attr)
{
	...
	if (fd < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", 
	...
	const char *devname, const char *attr)
{
	...
	if (f == NULL) {
		...
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "open %s failed errno=%d", filename, errno);
		return LIBUSB_ERROR_IO;
	}
	...
	fclose(f);
	if (r != 1) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "fscanf %s returned %d, errno=%d", attr, r, errno);
		return LIBUSB_ERROR_NO_DEVICE; /* For unplug race (trac #70) */
	}
	if (value < 0) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "%s contains a negative value", filename);
		return LIBUSB_ERROR_IO;
	}
...
static int sysfs_get_active_config(struct libusb_device *dev, int *config)
{
	...
	if (r < 0) {
		...
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "device unconfigured");
		*config = -1;
		return 0;
	}

	if (tmp[sizeof(tmp) - 1] != 0) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB",  "not null-terminated?");
		return LIBUSB_ERROR_IO;
	} else if (tmp[0] == 0) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB",  "no configuration value?");
		return LIBUSB_ERROR_IO;
	}

	num = strtol(tmp, &endptr, 10);
	if (endptr == tmp) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB",  "error converting '%s' to integer", tmp);
		return LIBUSB_ERROR_IO;
	}
	...
	const char *sys_name, int fd)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "getting address for device: %s detached: %d", sys_name, detached);
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "scan %s", sys_name);
	...
	*devaddr = (uint8_t) sysfs_attr;
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "bus=%d dev=%d", *busnum, *devaddr);
	...
	uint8_t descriptor_type, unsigned char *buffer, int size)
{
	...
	for (i = 0; size >= 0; i += header.bLength, size -= header.bLength) {
		...
		if (size < 2) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "short descriptor read %d/2", size);
			return LIBUSB_ERROR_IO;
		}
	...
	__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "bLength overflow by %d bytes", -size);
	return LIBUSB_ERROR_IO;
}
	...
	unsigned char *buffer, int size)
{
	...
	if (size < LIBUSB_DT_CONFIG_SIZE) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "short descriptor read %d/%d",
	...
	if (config.bDescriptorType != LIBUSB_DT_CONFIG) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "descriptor is not a config desc (type 0x%02x)",
	...
	if (priv->sysfs_dir && sysfs_has_descriptors) {
		...
		if (next != config.wTotalLength)
			__android_log_print(ANDROID_LOG_WARN, "LibUSB", "config length mismatch wTotalLength "
				  "%d real %d", config.wTotalLength, next);
		return next;
	} else {
		if (config.wTotalLength < LIBUSB_DT_CONFIG_SIZE) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "invalid wTotalLength %d",
				 config.wTotalLength);
			return LIBUSB_ERROR_IO;
		} else if (config.wTotalLength > size) {
			__android_log_print(ANDROID_LOG_WARN, "LibUSB", "short descriptor read %d/%d",
	...
	uint8_t devaddr, const char *sysfs_dir, int wrapped_fd)
{
	...
	if (sysfs_dir && sysfs_has_descriptors) {
		...
		if (r < 0) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "seek failed ret=%d errno=%d", r, errno);
			return LIBUSB_ERROR_IO;
		}
	}
	if (fd < 0)
	...
	do {
		...
		if (r < 0) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "read descriptor failed ret=%d errno=%d",
				 fd, errno);
			if (fd != wrapped_fd)
				close(fd);
			return LIBUSB_ERROR_IO;
	...
	if (priv->descriptors_len < DEVICE_DESC_LENGTH) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "short descriptor read (%d)",
	...
	if (fd < 0) {
		...
		__android_log_print(ANDROID_LOG_WARN, "LibUSB", "Missing rw usbfs access; cannot determine "
...
static int linux_get_parent_info(struct libusb_device *dev, const char *sysfs_dir)
{
	...
	if (NULL != (tmp = strrchr(parent_sysfs_dir, '.')) ||
		...
		*tmp = '\0';
		...
		__android_log_print(ANDROID_LOG_WARN, "LibUSB", "Can not parse sysfs_dir: %s, no parent info",
			  parent_sysfs_dir);
...
retry:
	...
	if (!dev->parent_dev && add_parent) {
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "parent_dev %s not enumerated yet, enumerating now",
			 parent_sysfs_dir);
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "Dev %p (%s) has parent %p (%s) port %d", dev, sysfs_dir,
	...
	uint8_t busnum, uint8_t devaddr, const char *sysfs_dir)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "busnum %d devaddr %d session_id %ld", busnum, devaddr,
	//	session_id);

	dev = usbi_get_device_by_session_id(ctx, session_id);
	if (dev) {
		/* device already exists in the context */
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "session_id %ld already exists", session_id);
		libusb_unref_device(dev);
		return LIBUSB_SUCCESS;
	}

	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "allocating new device for %d/%d (session %ld)",
...
void linux_device_disconnected(uint8_t busnum, uint8_t devaddr)
{
	...
	list_for_each_entry(ctx, &active_contexts_list, list, struct libusb_context) {
		...
		if (NULL != dev) {
			...
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "device not found for session %x", session_id);
		}
	}
	usbi_mutex_static_unlock(&active_contexts_lock);
}
...
static int usbfs_scan_busdir(struct libusb_context *ctx, uint8_t busnum)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "%s", dirpath);
	dir = opendir(dirpath);
	if (!dir) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "opendir '%s' failed, errno=%d", dirpath, errno);
	...
	while ((entry = readdir(dir))) {
		...
		if (devaddr == 0) {
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "unknown dir entry %s", entry->d_name);
			continue;
		}

		if (linux_enumerate_device(ctx, busnum, (uint8_t) devaddr, NULL)) {
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "failed to enumerate dir entry %s", entry->d_name);
			continue;
		}
...
static int usbfs_get_device_list(struct libusb_context *ctx)
{
	...
	if (!buses) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "opendir buses failed errno=%d", errno);
		return LIBUSB_ERROR_IO;
	}
...
static int sysfs_get_device_list(struct libusb_context *ctx)
{
	...
	if (!devices) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "opendir devices failed errno=%d", errno);
		return r;
	}
...
static int initialize_handle(struct libusb_device_handle *handle, int fd)
{
	...
	if (r < 0) {
		if (errno == ENOTTY)
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "getcap not available");
		else
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "getcap failed (%d)", errno);
		hpriv->caps = 0;
		if (supports_flag_zero_packet)
	...
	struct libusb_device_handle *handle, int fd)
{
	...
	if (r < 0) {
		...
		if (r < 0) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "connectinfo failed (%d)", errno);
			return LIBUSB_ERROR_IO;
		}
	 ...
	 * connected devices. */
	__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "allocating new device for fd %d", fd);
	...
	if (fd < 0) {
		if (fd == LIBUSB_ERROR_NO_DEVICE) {
			...
			if (handle->dev->attached) {
				__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "open failed with no device, but device still attached");
	...
	int *config)
{
	...
	if (*config == -1) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "device unconfigured");
		*config = 0;
	}
...
static int op_set_configuration(struct libusb_device_handle *handle, int config)
{
	...
	if (r) {
		...
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "failed, error %d errno %d", r, errno);
		return LIBUSB_ERROR_OTHER;
	}
	...
	size_t len)
{
	...
	if (buffer == MAP_FAILED) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "alloc dev mem failed errno %d",
			errno);
		return NULL;
	}
	return buffer;
	...
	unsigned char *buffer, size_t len)
{
	if (munmap(buffer, len) != 0) {
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "free dev mem failed errno %d",
			errno);
		return LIBUSB_ERROR_OTHER;
	} else {
		return LIBUSB_SUCCESS;
...
static int discard_urbs(struct usbi_transfer *itransfer, int first, int last_plus_one)
{
	...
	for (i = last_plus_one - 1; i >= first; i--) {
		...
		if (EINVAL == errno) {
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "URB not found --> assuming ready to be reaped");
			if (i == (last_plus_one - 1))
				ret = LIBUSB_ERROR_NOT_FOUND;
		} else if (ENODEV == errno) {
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "Device not found for URB --> assuming ready to be reaped");
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "need %d urbs for new transfer with length %d", num_urbs,
	//	transfer->length);
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (r < 0) {
			...
			if (i == 0) {
				__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "first URB failed, easy peasy");
				free(urbs);
				tpriv->urbs = NULL;
				return r;
			}
			...
			//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "reporting successful submission but waiting for %d "
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "need %d %dk URBs for transfer", num_urbs, MAX_ISO_BUFFER_LENGTH / 1024);
	...
	struct usbfs_urb *urb)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "handling completion status %d of bulk urb %d/%d", urb->status,
	...
	if (tpriv->reap_action != NORMAL) {
		...
		__android_log_print(ANDROID_LOG_WARN, "LibUSB", "abnormal reap: urb status %d", urb->status);
		...
		if (urb->actual_length > 0) {
			...
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "received %d bytes of surplus data", urb->actual_length);
			if (urb->buffer != target) {
				__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "moving surplus data from offset %d to offset %d",
		...
		if (tpriv->num_retired == tpriv->num_urbs) {
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "abnormal reap: last URB handled, reporting");
	...
	case -ENODEV:
	case -ESHUTDOWN:
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "device removed");
		tpriv->reap_status = LIBUSB_TRANSFER_NO_DEVICE;
		goto cancel_remaining;
	case -EPIPE:
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "detected endpoint stall");
		if (tpriv->reap_status == LIBUSB_TRANSFER_COMPLETED)
			tpriv->reap_status = LIBUSB_TRANSFER_STALL;
		goto cancel_remaining;
	case -EOVERFLOW:
		/* overflow can only ever occur in the last urb */
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "overflow, actual_length=%d", urb->actual_length);
		if (tpriv->reap_status == LIBUSB_TRANSFER_COMPLETED)
	...
	case -ECOMM:
	case -ENOSR:
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "low level error %d", urb->status);
	...
	if (urb_idx == tpriv->num_urbs - 1) {
		//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "last URB in transfer --> complete!");
		goto completed;
	} else if (urb->actual_length < urb->buffer_length) {
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "short transfer %d/%d --> complete!",
	...
	struct usbfs_urb *urb)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "handling completion status %d of iso urb %d/%d", urb->status,
	//	urb_idx, num_urbs);
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		case -EPIPE:
			...
			break;
		case -EOVERFLOW:
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "overflow error");
		...
		case -ENOSR:
		case -EXDEV:
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "low-level USB error %d", urb_desc->status);
	...
	if (tpriv->reap_action != NORMAL) { /* cancelled or submit_fail */
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "CANCEL: urb status %d", urb->status);

		if (tpriv->num_retired == num_urbs) {
			__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "CANCEL: last URB handled, reporting");
			free_iso_urbs(tpriv);
	...
	struct usbfs_urb *urb)
{
	...
	usbi_mutex_lock(&itransfer->lock);
	//__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "handling completion status %d", urb->status);
	...
	case -ESHUTDOWN:
		...
		break;
	case -EPIPE:
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "unsupported control request");
		status = LIBUSB_TRANSFER_STALL;
		break;
	case -EOVERFLOW:
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "control overflow error");
	...
	case -ECOMM:
	case -ENOSR:
		__android_log_print(ANDROID_LOG_DEBUG, "LibUSB", "low-level bus error occurred");
		status = LIBUSB_TRANSFER_ERROR;
		break;
	default:
		usbi_warn(ITRANSFER_CTX(itransfer),
...
static int reap_for_handle(struct libusb_device_handle *handle)
{
	...
	if (r < 0) {
		...
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "reap failed error %d errno=%d",
			r, errno);
		return LIBUSB_ERROR_IO;
	}
	...
	//__android_log_print(ANDROID_LOG_VERBOSE, "LibUSB", "urb type=%d status=%d transferred=%d", urb->type, urb->status,
	//	urb->actual_length);
	...
	default:
		__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "unrecognised endpoint type %x",
			transfer->type);
		return LIBUSB_ERROR_OTHER;
	}
}
	...
	struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		if (!hpriv || hpriv->fd != pollfd->fd) {
			__android_log_print(ANDROID_LOG_ERROR, "LibUSB", "cannot find handle for fd %d",
				 pollfd->fd);
			continue;
		}
